...
class GetOutstandingApi {
  ...
  String? message;
  GetOutstandingApi(
      {this.outstandhead, this.exception, this.message, this.stcode});

  static Future<GetOutstandingApi> getData(String meth) async {
    // Sellerkit_Flexi/v2/GetAllOutstandings

    Responce res = Responce();
    res = await ServiceGet.callApi(meth, Utils.token!);
    Map<String, dynamic> jsons = {};
    if (res.resCode! <= 210 && res.resCode! >= 200) {
      return GetOutstandingApi.fromJson(jsonDecode(res.responceBody!), res);
    }
    return GetOutstandingApi.fromJson(jsons, res);
  }

  factory GetOutstandingApi.fromJson(Map<String, dynamic> jsons, Responce res) {
    if (res.resCode! <= 210 && res.resCode! >= 200) {
      if (jsons['data'] != null) {
  ...
  factory outstandingheader.fromJson(Map<String, dynamic> jsons) {
    if (jsons['OutstandingMaster'] != null &&
      ...
      return outstandingheader(
          outstanddata: datalist1, outstandline: datalist2);
    } else {
      return outstandingheader(outstanddata: null, outstandline: null);
    }
  }
}
