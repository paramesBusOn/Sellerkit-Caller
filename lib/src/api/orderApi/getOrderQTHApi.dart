...
class GetOrderDetailsQTHApi {
  ...
  static Future<GetOrderDetailsQTHApi> getData(
    String meth,
  ) async {
    // SkClientPortal/getOrderQUT2?Docentry=$docentry
    Responce res = Responce();
    res = await ServiceGet.callApi(meth, Utils.token!);
    Map<String, dynamic> json = {};
    if (res.resCode! <= 210 && res.resCode! >= 200) {
      return GetOrderDetailsQTHApi.fromJson(jsonDecode(res.responceBody!), res);
    }
    return GetOrderDetailsQTHApi.fromJson(json, res);
  }

  factory GetOrderDetailsQTHApi.fromJson(
      Map<String, dynamic> jsons, Responce res) {
    if (res.resCode! <= 210 && res.resCode! >= 200) {
      if (jsons != null || jsons.isNotEmpty) {
        var list = json.decode(jsons["data"]) as Map<String, dynamic>;
        return GetOrderDetailsQTHApi(
...
class GetOrderDeatilsheaderData {
  ...
  GetOrderDeatilsheaderData(
      {required this.OrderDeatilsQTHData, required this.OrderDeatilsQTLData});
